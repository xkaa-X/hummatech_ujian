        ...
        <div class="flex items-center justify-between gap-4">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-400">
                    Tambahkan catatan tabungan untuk wishlist: <span class="text-indigo-400 font-semibold">{{ $wishlist->nama_wishlist }}</span>
                </p>
            </div>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-gray-800 border border-gray-700 shadow-sm sm:rounded-2xl p-6">
                ...
                <form action="{{ route('wishlist-detail.store', $wishlist->id) }}" method="POST" class="space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <input
                            ...
                            value="{{ old('kategori') }}"
                            class="mt-1 block w-full rounded-lg border-gray-600 bg-white text-gray-900 placeholder-gray-400 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                            placeholder="Contoh: Uang Jajan, Bonus, dll"
                            required
                        />
                        @error('kategori')
                            <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
